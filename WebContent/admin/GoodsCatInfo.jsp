				...
				<table class="layui-table">
					...
					<tbody>
						<%
							...
							String sql = "select * from goodscatinfo t where 1=1 ";
							...
							List<Category> categories = BaseDao.executeJDBCSQLQuerys(sql, params, Category.class);
							SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
							String parentCar = "select * from parentcatinfo t where t.parentCatID=?";
							for(Category category : categories){
								params = new ArrayList<Object>();
								params.add(category.getParentCatID());
								ParentCat parentCat = BaseDao.executeJDBCSQLQuery(parentCar, params, ParentCat.class);
						%>
							<tr>
								<td><%=parentCat.getParentCatName() %></td>
								<td><%=category.getCategoryName() %></td>
								<td><%=category.getCategoryText() %></td>
								<td><%=sdf.format(category.getCategoryTime()) %></td>
								<td><a
									class="layui-btn layui-btn-danger layui-btn-mini news_del"
									data-id="1" onclick="delFun('<%=category.getCategoryID() %>')"><i class="layui-icon"></i> 删除</a></td>
							</tr>
						 <%
							}
						 %>
